...
function generateHiddenInputsTemplate(newId, costTitle, costPrice, costDescription = '', costAirportFee = false) {
    ...
    return `
        <div id="hidden-input-${newId}">
            ...
            <input class="addAirportFee" data-id="${newId}" type="checkbox" id="addAirportFee-${newId}" name="costs[${newId}][has_airport_service_fee]" ${costAirportFee === true ? "checked" : ""} style="display: none">
    ...
    return `
            <div class="cost-field" id="${newId}">
                <div class="row">
                    ...
                    <div class="col-12 col-md-6 col-lg-6 mt-4 align-content-center">
                        <div class="form-check mt-3 mb-2">
                            <label class="form-check-label" for="addAirportFee-${newId}">
                                افزودن به هزینه خدمات فرودگاهی
                            </label>
                            <input class="form-check-input addAirportFee" data-id="${newId}" type="checkbox" id="addAirportFee-${newId}" name="costs[${newId}][has_airport_service_fee]" ${addAirportFee === true || addAirportFee === 'true' ? "checked" : ""}>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6 col-lg-6 mt-4" id="description-container-${newId}">
                        ...
                        <textarea type="text" id="description-${newId}" name="costs[${newId}][description]" class="form-control custom-elements" placeholder="..." rows="4">${costDescription ?? ''}</textarea>
                    </div>
...
function initEventHandlers() {
    ...
    handleUsersInputKeyUp()

    handleCaseInputClick();
    handleCasesInputKeyUp();

    handleSearchedUserClick();
    handleSearchedCaseClick();
    handleAccountClick();

    handleWeightElementBlur();
